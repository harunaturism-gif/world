...
export function createAppSessionConfig(environment) {
...
export function deriveInternalUser(worldSessionId, identitySecret) {
...
export function signApplicationSession(user, sessionSecret, options = {}) {
    ...
    return sign({ username: user.username, iat: now }, sessionSecret, {
        algorithm: 'HS256',
...
export function verifyApplicationSession(token, sessionSecret, now = Math.floor(Date.now() / 1000)) {
    ...
    try {
        ...
        if (!isRecord(payload))
            return null;
        if (Object.keys(payload).sort().join(',') !== EXPECTED_PAYLOAD_KEYS.join(','))
            return null;
        if (payload.iss !== APP_SESSION_ISSUER || payload.aud !== APP_SESSION_AUDIENCE)
            return null;
        if (typeof payload.sub !== 'string' || !INTERNAL_USER_ID_PATTERN.test(payload.sub))
            return null;
        if (typeof payload.username !== 'string' || payload.username !== usernameForUserId(payload.sub))
            return null;
        if (typeof payload.jti !== 'string' || !JWT_ID_PATTERN.test(payload.jti))
            return null;
        if (!Number.isInteger(payload.iat) || !Number.isInteger(payload.exp))
            return null;
        if (payload.exp <= payload.iat
            || payload.exp - payload.iat > APP_SESSION_LIFETIME_SECONDS
            || payload.iat > now + 60) {
            return null;
        }
...
function cookieAttributes(isProduction) {
    return [
        'Path=/',
        'HttpOnly',
        ...(isProduction ? ['Secure'] : []),
...
export function extractSessionToken(cookieHeader, isProduction) {
    ...
    if (matches.length !== 1 || !JWT_PATTERN.test(matches[0] ?? ''))
        return null;
    return matches[0] ?? null;
}
export function createSanitizedAuthResponse(user) {
    if (!isInternalUser(user))
        throw new Error('Invalid sanitized user');
    return { verified: true, user: { ...user } };
}
export function isExpectedBrowserOrigin(origin, appOrigin) {
