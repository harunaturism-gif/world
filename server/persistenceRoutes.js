...
function userForRequest(request) {
    ...
    if (!user)
        throw new Error('Missing authenticated persistence context');
    return user;
}
function persistenceFailure(response) {
...
export function createPersistenceRouter(options) {
    ...
    router.get('/profiles/:userId/counts', async (request, response) => {
        if (!isInternalUserId(request.params.userId))
            return response.status(400).json({ error: 'Invalid profile request' });
        try {
            return response.json(await options.repository.getProfileCounts(request.params.userId));
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.get('/follows/:targetId', async (request, response) => {
        if (!isInternalUserId(request.params.targetId))
            return response.status(400).json({ error: 'Invalid follow request' });
        try {
            return response.json({ following: await options.repository.isFollowing(userForRequest(request).id, request.params.targetId) });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.put('/follows/:targetId', async (request, response) => {
        ...
        try {
            return response.json({ following: await options.repository.setFollowing(actor.id, request.params.targetId, true) });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.delete('/follows/:targetId', async (request, response) => {
        ...
        try {
            return response.json({ following: await options.repository.setFollowing(actor.id, request.params.targetId, false) });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.get('/feed', async (_request, response) => {
        try {
            return response.json({ posts: await options.repository.getFeed() });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.post('/posts', async (request, response) => {
        ...
        try {
            return response.status(201).json({ post: await options.repository.createPost(userForRequest(request), input) });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.put('/posts/:postId/like', async (request, response) => {
        ...
        if (!postId || (request.body !== undefined && Object.keys(request.body).length > 0)) {
            return response.status(400).json({ error: 'Invalid like request' });
        }
        try {
            return response.json({ likes: await options.repository.likePost(userForRequest(request).id, postId) });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.post('/rooms', async (request, response) => {
        ...
        if (!input)
            return response.status(400).json({ error: 'Invalid room' });
        try {
            return response.status(201).json({ room: await options.repository.createRoom(userForRequest(request), input) });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.get('/avatar', async (request, response) => {
        try {
            return response.json({ appearance: await options.repository.getAvatar(userForRequest(request).id) });
        }
        catch {
            return persistenceFailure(response);
        }
    ...
    router.put('/avatar', async (request, response) => {
        ...
        if (!appearance)
            return response.status(400).json({ error: 'Invalid avatar' });
        try {
            return response.json({ appearance: await options.repository.updateAvatar(userForRequest(request).id, appearance) });
        }
        catch {
            return persistenceFailure(response);
        }
...
//# sourceMappingURL=persistenceRoutes.js.map