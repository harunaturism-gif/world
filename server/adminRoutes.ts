...
const requestContexts = new WeakMap<Request, AdminRequestContext>();

function contextForRequest(request: Request): AdminRequestContext {
  const context = requestContexts.get(request);
  if (!context) throw new Error('Missing admin request context');
  return context;
}

function adminFailure(response: Response, error: unknown) {
  if (error instanceof AdminVersionConflictError) return response.status(409).json({ error: 'Version conflict' });
  if (error instanceof AdminInvariantError) return response.status(409).json({ error: 'Administrative invariant rejected' });
...
export function createAdminRouter(options: {
  ...
  router.options('{*path}', (request, response) => {
    ...
    if (!options.appSessionConfig || !options.repository) return response.status(503).json({ error: 'Admin service unavailable' });
    if (request.headers.origin !== options.appSessionConfig.appOrigin) return response.status(403).json({ error: 'Origin not allowed' });
  ...
  const requireCapability = (capability: AdminCapability) => (request: Request, response: Response, next: NextFunction) => {
    if (!hasCapability(contextForRequest(request).session, capability)) return response.status(403).json({ error: 'Admin capability denied' });
    return next();
  };
  ...
  router.get('/rooms', requireCapability('rooms:write'), async (_request, response) => {
    ...
    catch (error) { return adminFailure(response, error); }
  });
  ...
  router.patch('/rooms/:roomId', requireCapability('rooms:write'), async (request, response) => {
    ...
    if (!input) return response.status(400).json({ error: 'Invalid room metadata' });
    try { return response.json({ room: await options.repository!.updateRoomMetadata(contextForRequest(request).user.id, input) }); }
    catch (error) { return adminFailure(response, error); }
  });
  ...
  router.get('/rooms/:roomId/layout', requireCapability('rooms:write'), async (request, response) => {
    ...
    try { return response.json({ published: await options.repository!.getRoomLayout(request.params.roomId) }); }
    catch (error) { return adminFailure(response, error); }
  });
  ...
  router.put('/rooms/:roomId/layout', requireCapability('rooms:write'), async (request, response) => {
    ...
    if (!input) return response.status(400).json({ error: 'Invalid room layout' });
    try { return response.json({ published: await options.repository!.publishRoomLayout(contextForRequest(request).user.id, input) }); }
    catch (error) { return adminFailure(response, error); }
  });
  ...
  router.delete('/rooms/:roomId/layout', requireCapability('rooms:write'), async (request, response) => {
    ...
    if (!input) return response.status(400).json({ error: 'Invalid layout reset' });
    try {
  ...
  router.get('/roles', requireCapability('roles:manage'), async (_request, response) => {
    try { return response.json({ roles: await options.repository!.listRoles() }); }
    catch (error) { return adminFailure(response, error); }
  });

  router.put('/roles/:userId', requireCapability('roles:manage'), async (request, response) => {
    if (!isInternalUserId(request.params.userId)) return response.status(400).json({ error: 'Invalid role assignment' });
    const role = parseRoleInput(request.body);
    if (!role) return response.status(400).json({ error: 'Invalid role assignment' });
    try { return response.json({ assignment: await options.repository!.setRole(contextForRequest(request).user.id, request.params.userId, role) }); }
    catch (error) { return adminFailure(response, error); }
  });

  router.delete('/roles/:userId', requireCapability('roles:manage'), async (request, response) => {
    if (!isInternalUserId(request.params.userId)) return response.status(400).json({ error: 'Invalid role assignment' });
    if (request.body !== undefined && Object.keys(request.body as object).length > 0) return response.status(400).json({ error: 'Invalid role assignment' });
    try {
