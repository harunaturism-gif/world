...
const ROLE_VALUES = new Set(['owner', 'editor']);
...
function validPosition(value) {
    if (!isPlainObject(value) || !hasOnlyKeys(value, ['x', 'y', 'z']))
        return false;
...
function safeJson(value, depth = 0) {
    ...
    if (typeof value === 'string')
        return value.length <= 1_024 && !/session_[0-9a-fA-F]{128}/.test(value);
    if (typeof value === 'number')
        return Number.isFinite(value) && Math.abs(value) <= 0xffffff;
    if (Array.isArray(value))
        return value.length <= 4_096 && value.every((entry) => safeJson(entry, depth + 1));
...
function validInteraction(value) {
    if (value === undefined)
        return true;
    if (!isPlainObject(value) || !hasOnlyKeys(value, ['title', 'description', 'actionLabel', 'action', 'targetId', 'icon', 'effect', 'facing']))
        return false;
...
function validRoomObject(value) {
    if (!isPlainObject(value) || !safeJson(value))
        return false;
    if (typeof value.id !== 'string' || !IDENTIFIER.test(value.id))
        return false;
    if (value.catalogId !== undefined && (typeof value.catalogId !== 'string' || !CATALOG_ID.test(value.catalogId)))
        return false;
    if (typeof value.asset !== 'string' || value.asset.length > 300 || !value.asset.startsWith('/assets/world/'))
        return false;
    if (typeof value.category !== 'string' || !OBJECT_CATEGORIES.has(value.category)
        ...
        return false;
    if (value.direction !== undefined && !finiteInteger(value.direction, 0, 359))
        return false;
    if (value.state !== undefined && (typeof value.state !== 'string' || !CATALOG_ID.test(value.state)))
        return false;
...
function validLayout(layout, roomId) {
    if (!isPlainObject(layout) || !safeJson(layout))
        return false;
    if (!hasOnlyKeys(layout, ['id', 'name', 'geometry', 'floor', 'objects', 'contextObjects', 'avatars', 'spaces', 'events', 'districts', 'studio', 'ui']))
        return false;
    if (layout.id !== roomId || typeof layout.name !== 'string' || layout.name.length < 1 || layout.name.length > 80)
        return false;
    if (!isPlainObject(layout.geometry) || !hasExactKeys(layout.geometry, ['cells', 'walls', 'exits', 'spawn', 'maxStepHeight']))
        return false;
    ...
    if (!Array.isArray(cells) || cells.length < 1 || cells.length > 4_096
        ...
        || !validPosition(spawn) || !finiteNumber(maxStepHeight, 0, 100))
        return false;
    ...
    if (!walls.every((wall) => isPlainObject(wall) && safeJson(wall) && typeof wall.id === 'string' && IDENTIFIER.test(wall.id)
        && validPosition(wall.from) && validPosition(wall.to)))
        return false;
    if (!exits.every((exit) => isPlainObject(exit) && safeJson(exit) && typeof exit.id === 'string' && IDENTIFIER.test(exit.id)
        && validPosition(exit.position) && typeof exit.targetRoomId === 'string' && isRoomId(exit.targetRoomId)))
        return false;
    if (!Array.isArray(layout.objects) || layout.objects.length > 512 || !layout.objects.every(validRoomObject))
        return false;
    if (layout.contextObjects !== undefined && (!Array.isArray(layout.contextObjects) || layout.contextObjects.length > 256 || !layout.contextObjects.every(validRoomObject)))
        return false;
    if (!Array.isArray(layout.avatars) || layout.avatars.length > 128 || !layout.avatars.every(validAvatar))
        return false;
    if (layout.events !== undefined && (!Array.isArray(layout.events) || layout.events.length > 128 || !layout.events.every(validEvent)))
        return false;
    if (layout.spaces !== undefined && (!Array.isArray(layout.spaces) || layout.spaces.length > 128))
        return false;
    if (layout.districts !== undefined && (!Array.isArray(layout.districts) || layout.districts.length > 128))
        return false;
    const worldIds = [
        ...layout.objects.map((entry) => entry.id),
        ...(Array.isArray(layout.contextObjects) ? layout.contextObjects.map((entry) => entry.id) : []),
        ...layout.avatars.map((entry) => entry.id),
    ];
    if (new Set(worldIds).size !== worldIds.length)
        return false;
    const cellIds = cells.map((cell) => `${cell.x}:${cell.y}`);
...
export function createAdminConfig(environment) {
    const isDevelopment = environment.NODE_ENV === 'development';
    const developmentAdminEnabled = environment.ENABLE_DEV_ADMIN === 'true';
    if (!isDevelopment && developmentAdminEnabled)
        return null;
    if (environment.ENABLE_DEV_ADMIN !== undefined && !['true', 'false'].includes(environment.ENABLE_DEV_ADMIN))
        return null;
    const raw = environment.ADMIN_BOOTSTRAP_USER_IDS ?? '';
    if (raw !== raw.trim())
        return null;
    const entries = raw === '' ? [] : raw.split(',');
    if (entries.some((entry) => !isInternalUserId(entry)) || new Set(entries).size !== entries.length)
        return null;
    return { bootstrapUserIds: new Set(entries), developmentAdminEnabled };
}
export function shouldBootstrapOwner(config, userId) {
...
export async function authenticateAdminRequest(input) {
    if (!input.appSessionConfig || !input.repository)
        return { accepted: false, statusCode: 503 };
    if (input.originHeader !== input.appSessionConfig.appOrigin)
...
export function parseRoleInput(value) {
    if (!isPlainObject(value) || !hasExactKeys(value, ['role']))
        return null;
    return typeof value.role === 'string' && ROLE_VALUES.has(value.role) ? value.role : null;
}
export function parseRoomMetadataInput(value, routeRoomId) {
    const keys = ['capacity', 'isPublic', 'landStatus', 'left', 'name', 'priceHum', 'roomId', 'top', 'type'];
    if (!isRoomId(routeRoomId) || !isPlainObject(value) || !hasExactKeys(value, keys) || value.roomId !== routeRoomId)
        return null;
    if (typeof value.name !== 'string' || value.name !== value.name.trim() || value.name.length < 1 || value.name.length > 80
        ...
        || (value.priceHum !== null && !finiteNumber(value.priceHum, 0, 1_000_000_000)))
        return null;
    return value;
}
export function parsePublishRoomLayoutInput(value, routeRoomId) {
    if (!isRoomId(routeRoomId) || !isPlainObject(value) || !hasExactKeys(value, ['expectedVersion', 'layout', 'roomId']) || value.roomId !== routeRoomId)
        return null;
    if (!finiteInteger(value.expectedVersion, 0, Number.MAX_SAFE_INTEGER))
        return null;
    let size;
    try {
        size = Buffer.byteLength(JSON.stringify(value.layout), 'utf8');
    }
    catch {
        return null;
    }
    if (size > MAX_LAYOUT_BYTES || !validLayout(value.layout, routeRoomId))
        return null;
    return { expectedVersion: value.expectedVersion, layout: value.layout, roomId: routeRoomId };
}
export function parseResetRoomLayoutInput(value, routeRoomId) {
    if (!isRoomId(routeRoomId) || !isPlainObject(value) || !hasExactKeys(value, ['expectedVersion', 'roomId']) || value.roomId !== routeRoomId)
        return null;
