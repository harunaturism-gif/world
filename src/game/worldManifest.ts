...
const action = (title: string, description: string, actionLabel: string, type: RoomInteraction['action'], icon: string, targetId?: string, effect?: RoomInteraction['effect'], facing?: RoomInteraction['facing']): RoomInteraction => ({ title, description, actionLabel, action: type, icon, targetId, effect, facing });
...
export const centralPlazaRoom: RoomDefinition = {
  ...
  objects: [
    ...
    placeFurniture('communityBoard', 'community-board', { x: 8.2, y: 5.8 }, { interactionPoint: { x: 6.8, y: 5.7 }, interaction: action('What’s On', 'DJ set now · creator meetup next · gallery walk tonight.', 'Open schedule', 'inspect', '!', undefined, 'read-events') }),
