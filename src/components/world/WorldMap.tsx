...
export function WorldMap({ onEnterRoom, currentUser }: WorldMapProps) {
  ...
  useEffect(() => {
    let active = true;
    const refresh = () => {
      void RoomService.getRooms().then((data) => {
        if (active) setSpaces(data);
      });
    };

    refresh();
    window.addEventListener(ROOM_MAP_UPDATED_EVENT, refresh);
    return () => {
      active = false;
      window.removeEventListener(ROOM_MAP_UPDATED_EVENT, refresh);
    };
  }, []);
          ...
          <div className="relative bg-zinc-900/90 border border-zinc-800 backdrop-blur-sm p-3 rounded-2xl shadow-xl flex items-center gap-3 group-hover:border-blue-500/50 transition-colors">
            ...
            <div className="text-left whitespace-nowrap">
              ...
              <div className={`mt-1 text-[10px] font-bold uppercase tracking-wide ${
                space.land_status === 'available' ? 'text-amber-300' : 'text-zinc-600'
              }`}>
                {space.land_status === 'available'
                  ? `${space.price_hum?.toLocaleString() ?? 'Price pending'} HUM`
                  : space.land_status ?? 'public'}
              </div>
            </div>
