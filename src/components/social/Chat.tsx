...
export function Chat({ onSendMessage, incomingMessage }: ChatProps) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => setPreviewVisible(false), 4600);
  ...
  return (
    <div className="pointer-events-auto relative flex h-16 items-center border-t border-white/[0.06] bg-gradient-to-t from-[#07171c]/97 to-[#10252d]/82 px-2.5 shadow-[0_-12px_34px_rgba(3,14,18,.28)] backdrop-blur-2xl sm:px-4">
      {latestMessage && previewVisible ? (
      ...
      <form onSubmit={handleSend} className="group mx-auto flex w-full max-w-3xl items-center gap-1.5 rounded-2xl border border-white/10 bg-[#102c33]/90 p-1.5 shadow-[inset_0_1px_rgba(255,255,255,.05),0_8px_28px_rgba(0,0,0,.2)] transition focus-within:border-emerald-200/35 focus-within:bg-[#12343c]">
        ...
        <button type="button" aria-label="Add wave emoji" onClick={() => setInput((value) => `${value}👋`)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-white/45 transition hover:bg-white/10 hover:text-amber-200">
          <Smile size={18} />
        </button>
        <div className="relative min-w-0 flex-1">
          <input
            ...
            className="h-9 w-full min-w-0 bg-transparent px-1 text-[14px] font-medium text-white placeholder:text-white/35 focus:outline-none"
          />
        ...
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-100 to-amber-300 text-[#10252d] shadow-[0_5px_16px_rgba(251,191,36,.2)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-35"
        >
          <Send size={17} />
        </button>
      </form>
